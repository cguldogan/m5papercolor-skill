...
7. **`examples/ir-tv-blaster/`** — IR LED on G48 fires correctly. Both **raw `digitalWrite` 38 kHz square wave** and **IRremote v4 with the M5Stack `begin(DISABLE_LED_FEEDBACK) / setSendPin(48)` pattern** produce camera-visible pulses. The canonical M5Stack pattern is correct; when `IR_SEND_PIN` is not defined as a build flag, the library's ESP32 LEDC code reads `IrSender.sendPin` at runtime. See [input.md](references/input.md).
...
- [`examples/ir-samsung-debug/`](examples/ir-samsung-debug/) — focused Samsung tester with **Button B = raw 38 kHz square wave** mode for library-bypass debugging. Useful when a TV doesn't react: confirms whether the issue is the IR LED, the library, or just the wrong code-database entries.
